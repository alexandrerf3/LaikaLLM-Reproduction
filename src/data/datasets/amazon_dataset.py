...
from src.utils import PrintWithSpin, dict_list2list_dict, list_dict2dict_list
...
class AmazonDataset(LaikaDataset):
    ...
    def __init__(
        ...
        with open(
            os.path.join(
                RAW_DATA_DIR, "AmazonDataset", self.dataset_name, "datamaps.json"
            ),
            "r",
        ) as f:
        ...
        with open(
            os.path.join(
                RAW_DATA_DIR, "AmazonDataset", self.dataset_name, "user_id2name.pkl"
            ),
            "rb",
        ) as f:
        ...
        with PrintWithSpin("Extracting side-information"):
            for meta_content in parse(
                os.path.join(
                    RAW_DATA_DIR, "AmazonDataset", self.dataset_name, "meta.json.gz"
                )
            ):
        ...
        with PrintWithSpin("Creating tabular data"):
            for user_id, item_list_ids in user_items.items():
                ...
                user_name_col_repeated = [
                    self.user_id2name.get(user_id, "")
                    for _ in range(len(item_list_ids))
                ]
                user_asin_col_repeated = [
                    self.id2user.get(user_id, "") for _ in range(len(item_list_ids))
                ]
            ...
            if not os.path.isfile(raw_data_zip_path):
                ...
                try:
                    ...
                    raise FileURLRetrievalError(
                        "Permission denied to download the dataset or dataset removed!\n"
                        ...
                        "from the link above and move it into 'data/raw' folder!"
                    ) from None
                    ...
                    for subfolder in subfolder_to_extract:
                        ...
                        for path_in_zip in zip_ref.namelist():
                            if path_in_zip.startswith(dir_to_extract):
                                zip_ref.getinfo(path_in_zip).filename = "/".join(
                                    path_in_zip.split("/")[1:]
                                )
                                zip_ref.extract(
                                    member=path_in_zip, path=raw_data_folder_out
                                )
    ...
    def split_data(self, exploded_data_df: pd.DataFrame):
        ...
        train_set = (
            groupby_obj.nth[:-2]
            .groupby(by=["user_id", "user_name", "user_asin"])
            .agg(list)
            .reset_index()
        )
        ...
        # gt_item: [7]
        input_val_set = groupby_obj.nth[:-2].rename(
            columns={
        ...
        input_val_set = (
            input_val_set.groupby(by=["user_id", "user_name", "user_asin"])
            .agg(list)
            .reset_index()
        )

        gt_val_set = groupby_obj.nth[-2].rename(
            columns={
        ...
        gt_val_set = (
            gt_val_set.groupby(by=["user_id", "user_name", "user_asin"])
            .agg(list)
            .reset_index()
        )

        val_set = input_val_set.merge(
            gt_val_set, on=["user_id", "user_name", "user_asin"]
        )

        # if sequence is -> [1 2 3 4 5 6 7 8], TEST SET will have
        # input_sequence: [1 2 3 4 5 6 7]
        # gt_item: [8]
        input_test_set = groupby_obj.nth[:-1].rename(
            columns={
                "item_sequence": "input_item_seq",
                "rating_sequence": "input_rating_seq",
                "description_sequence": "input_description_seq",
        ...
        input_test_set = (
            input_test_set.groupby(by=["user_id", "user_name", "user_asin"])
            .agg(list)
            .reset_index()
        )

        gt_test_set = groupby_obj.nth[-1].rename(
            columns={
                "item_sequence": "gt_item",
                "rating_sequence": "gt_rating",
                "description_sequence": "gt_description",
        ...
        # features. We are performing Leave One Out, so we are sure there is only one item
        gt_test_set = (
            gt_test_set.groupby(by=["user_id", "user_name", "user_asin"])
            .agg(list)
            .reset_index()
        )

        test_set = input_test_set.merge(
            gt_test_set, on=["user_id", "user_name", "user_asin"]
        )
    ...
    def sample_train_sequence(batch: Dict[str, list]) -> Dict[str, list]:
        ...
        for sample in batch:
            ...
            if len(sample["item_sequence"]) < 2:
                raise ValueError(
                    f"{sample['user_id']} has less than 2 items in its order history, can't divide "
            ...
            sliding_size = random.randint(
                minimum_sliding_size, len(sample["item_sequence"]) - 1
            )

            start_index = random.randint(
                0, len(sample["item_sequence"]) - sliding_size - 1
            ...
            single_out_dict["input_item_seq"] = sample["item_sequence"][
                start_index:end_index
            ]
            single_out_dict["input_rating_seq"] = sample["rating_sequence"][
                start_index:end_index
            ]
            single_out_dict["input_description_seq"] = sample["description_sequence"][
                start_index:end_index
            ]
            single_out_dict["input_categories_seq"] = sample["categories_sequence"][
                start_index:end_index
            ]
            single_out_dict["input_title_seq"] = sample["title_sequence"][
                start_index:end_index
            ]
            single_out_dict["input_price_seq"] = sample["price_sequence"][
                start_index:end_index
            ]
            single_out_dict["input_imurl_seq"] = sample["imurl_sequence"][
                start_index:end_index
            ]
            single_out_dict["input_brand_seq"] = sample["brand_sequence"][
                start_index:end_index
            ]

            single_out_dict["gt_item"] = [sample["item_sequence"][end_index]]
            single_out_dict["gt_rating"] = [sample["rating_sequence"][end_index]]
            single_out_dict["gt_description"] = [
                sample["description_sequence"][end_index]
            ]
            single_out_dict["gt_categories"] = [
                sample["categories_sequence"][end_index]
            ]
    ...
    def _read_sequential(self):
        ...
        with open(
            os.path.join(
                RAW_DATA_DIR, "AmazonDataset", self.dataset_name, "sequential_data.txt"
            )
        ) as f:
    ...
    def _read_ratings(self, user_items: dict):
        ...
        for rating_dict in (
            ratings_list["train"] + ratings_list["val"] + ratings_list["test"]
        ):
    ...
    def get_hf_datasets(
        self, merge_train_val: bool = False
        ...
        if merge_train_val is True:
            ...
            train_df = (
                groupby_obj.nth[:-1].groupby(by=["user_id"]).agg(list).reset_index()
            )
            train_hf_ds = Dataset.from_pandas(
                train_df, split=datasets.Split.TRAIN, preserve_index=False
            )
            ...
            val_hf_ds = Dataset.from_pandas(
                val_df, split=datasets.Split.VALIDATION, preserve_index=False
            )
            dataset_dict["train"] = train_hf_ds
            dataset_dict["validation"] = val_hf_ds

        test_hf_ds = Dataset.from_pandas(
            test_df, split=datasets.Split.TEST, preserve_index=False
        )
