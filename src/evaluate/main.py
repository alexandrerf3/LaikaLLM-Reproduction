...
from src import METRICS_DIR, MODELS_DIR, PROCESSED_DATA_DIR, GeneralParams
...
def eval_main(
    ...
    if dataset_size:
        test_set = Dataset.from_dict(test_set[:dataset_size])
    ...
    eval_task_dict = {
        LaikaTask.from_string(eval_task_str): [
            LaikaMetric.from_string(metric_str) for metric_str in metric_list_str
        ]
