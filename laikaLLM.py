...
from src.utils import IndentedDumper, init_wandb, seed_everything
...
def pretty_print_configuration(config: dict):
    ...
    print(
        yaml.dump(
            config["data_params"], default_flow_style=False, Dumper=IndentedDumper
        )
    )
    ...
    print(
        yaml.dump(
            config["model_params"], default_flow_style=False, Dumper=IndentedDumper
        )
    )
    ...
    print(
        yaml.dump(
            config["eval_params"], default_flow_style=False, Dumper=IndentedDumper
        )
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Main script to reproduce perform the experiments"
    )
    ...
    general_params, data_params, model_params, eval_params = parse_yml_config(
        args.config
    )
    ...
    if general_params.log_wandb:
        ...
        if "WANDB_API_KEY" not in os.environ:
            raise ValueError(
                'Cannot log run to wandb if environment variable "WANDB_API_KEY" is not present\n'
                "Please set the environment variable and add the api key for wandb\n"
            )

        if "WANDB_ENTITY" not in os.environ:
            raise ValueError(
                'Cannot log run to wandb if environment variable "WANDB_ENTITY" is not present\n'
    ...
    with init_wandb(
        project=general_params.wandb_project,
        name=general_params.exp_name,
        config=config_args,
